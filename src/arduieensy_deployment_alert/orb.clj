...
(defn- open [orb]
  (assoc orb :output-stream (open-teensy-output-stream)))
...
(defn start! []
  "Initialize the ambient orb and allow it to begin waiting for updates"
  (let [active-orb (open orb)]
    (do (send-off (agent active-orb) update!)
        active-orb)))